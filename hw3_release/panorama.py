...
def harris_corners(img, window_size=3, k=0.04):
    ...
    dx = filters.sobel_v(img)   #compute dx
    dy = filters.sobel_h(img)   #compute dy

    ### YOUR CODE HERE
    # from https://github.com/veraposeidon/CS131_Assignments/blob/master/hw3_release/panorama.py
    Ix2 = dx ** 2
    Iy2 = dy ** 2
    IxIy = dx* dy 
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        inliers = np.linalg.norm(output - matched1, axis=1)**2 < threshold
        ...
        for w in range(cols):
            for m in range(G_cells.shape[2]):
                for n in range(G_cells.shape[3]):
                    idx = int(theta_cells[h, w, m, n]//degrees_per_bin)
                    if idx ==9 :
                        idx = 8
                    cells[h, w, idx] += G_cells[h, w, m, n]
