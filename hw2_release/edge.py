...
def conv(image, kernel):
    ...
    for h in range(Hi):
        for w in range(Wi):
            ...
            img_slice = padded[h:h + Hk,w:w + Wk]
...
def non_maximum_suppression(G, theta):
    ...
    ### BEGIN YOUR CODE
    G_pad = np.pad(G,((1,1),(1,1)),mode = 'constant')
    for i in range(1,H+1):
        for j in range(1,W+1):
            # 计算方向
            rad = np.deg2rad(theta[i-1, j-1])
            h =int(np.around(np.sin(rad)))   # 行
            w =int(np.around(np.cos(rad)))   # 列
            #得到当前位置以及梯度方向的两个相邻值
            p1 = G_pad[i+h, j+w]
            p2 = G_pad[i-h, j-w]
            if(G_pad[i, j] > p1 and G_pad[i, j] > p2): # 如果当前位置是该方向的最大值，则保留
